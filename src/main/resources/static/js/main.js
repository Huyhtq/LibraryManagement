document.addEventListener('DOMContentLoaded', () => {
    ...
    // Handle Borrow Form with Search and Auto-fill
    ...
    borrowerNameInput.addEventListener('input', debounce(async () => {
        ...
        if (query) {
            const filteredBorrowers = borrowers.filter(b => b.name.toLowerCase().includes(query));
    ...
    bookNameInput.addEventListener('input', debounce(async () => {
        const query = bookNameInput.value.toLowerCase();
        bookSuggestions.innerHTML = '';
        bookSuggestions.style.display = query ? 'block' : 'none';

        if (query) {
            const filteredBooks = books.filter(b => b.title.toLowerCase().includes(query));
    ...
    // ... (giữ nguyên các hàm loadRecords, loadLoansToday, v.v.)
